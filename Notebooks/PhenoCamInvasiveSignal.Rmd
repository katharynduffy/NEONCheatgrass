...
```{r}
ONAQ_sh=get_pheno_ts(site='NEON.D15.ONAQ.DP1.00042', vegType='SH', roiID=1001, type='1day')
ONAQ_sh=ONAQ_sh[!is.na(gcc_90)]
ONAQ_sh$smoothed=smooth(ONAQ_sh$gcc_90)
ONAQ_gr=get_pheno_ts(site='NEON.D15.ONAQ.DP1.00042', vegType='GR', roiID=1000, type='1day')
...
df_sh <- read.table("NEON.D15.ONAQ.DP1.00042_SH_1001_1day.csv", header = TRUE, sep = ",")

# read in the transition date file
td_sh <- read.table("NEON.D15.ONAQ.DP1.00042_SH_1001_1day_transition_dates.csv",
                 header = TRUE,
                 sep = ",")
df_gr <- read.table("NEON.D15.ONAQ.DP1.00042_GR_1000_1day.csv", header = TRUE, sep = ",")

# read in the transition date file
td_gr <- read.table("NEON.D15.ONAQ.DP1.00042_GR_1000_1day_transition_dates.csv",
                 header = TRUE,
                 sep = ",")
```

```{r}
spring_sh <- td_sh[td_sh$direction == "rising" & td$gcc_value == "gcc_90",]
fall_sh <- td_sh[td_sh$direction == "falling" & td$gcc_value == "gcc_90",]
spring_gr <- td_gr[td_gr$direction == "rising" & td$gcc_value == "gcc_90",]
fall_gr <- td_gr[td_gr$direction == "falling" & td$gcc_value == "gcc_90",]
```
```{r}
p_sh = plot_ly() %>%
  add_trace(
